<template>
  <el-card class="module-card">
    ...
    <div class="module-header">
      <h3 class="module-title">九、教改论文</h3>
      <!-- 新增按钮：只在非审核模式下显示 -->
      <el-button 
        v-if="mode !== 'check'" 
        type="primary" 
    ...
    >
      ...
      <!-- 论文名称（文本输入） -->
      <el-table-column prop="paperName" label="论文名称" width="220" align="center">
        <template v-slot="scope">
          <el-input 
            ...
            class="edit-input"
          ></el-input>
          <span v-else>{{ scope.row.paperName || '-' }}</span>
        </template>
      </el-table-column>

      <!-- 发表日期（日期选择器） -->
      <el-table-column prop="publishDate" label="发表日期" width="180" align="center">
        <template v-slot="scope">
          <el-date-picker 
            v-if="scope.row.isEditing" 
      ...
      <el-table-column prop="level" label="级别" width="180" align="center">
        <template v-slot="scope">
          <el-select 
            ...
            class="edit-select"
          >
            <el-option label="一级期刊" value="一级期刊"></el-option>
            <el-option label="核心期刊" value="核心期刊"></el-option>
            <el-option label="其他期刊" value="其他期刊"></el-option>
          </el-select>
      ...
      <el-table-column label="论文原件（PDF/图片）" min-width="180" align="center">
        <template v-slot="scope">
...
export default {
  ...
  methods: {
    ...
    addRow() {
      this.tableData.push({
        id: null,
        paperName: '', // 论文名称（空字符串初始化）
        publishDate: '', // 发表日期（空字符串初始化）
        level: '一级期刊', // 级别（默认国家级）
        workload: 0, // 工作量（默认0，与科技创新一致）
        pdfUrl: '',
        isEditing: true
      });
    },
    ...
    async saveRow(row) {
      // 校验必填项（适配论文模块字段）
      if (!row.paperName.trim()) return ElMessage.warning('请输入论文名称');
      if (!row.publishDate) return ElMessage.warning('请选择发表日期');
      ...
      this.$emit('save-row', 'educationReformPaper', row); // 模块标识：educationReformPaper
    },
    ...
    deleteRow(rowId, index) {
      this.tableData.splice(index, 1);
      this.$emit('delete-row', 'educationReformPaper', rowId); // 模块标识：educationReformPaper
    },
