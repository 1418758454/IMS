<template>
  <el-card class="module-card">
    ...
    <div class="module-header">
      <h3 class="module-title">五、科技创新</h3>
      <!-- 新增按钮：只在非审核模式下显示 -->
      <el-button 
        v-if="mode !== 'check'" 
        type="primary" 
    ...
    >
      ...
      <!-- 项目名称（文本输入） -->
      <el-table-column prop="projectName" label="项目名称" width="220" align="center">
        <template v-slot="scope">
          <el-input 
      ...
      <!-- 结题年份（数字输入，限制为年份格式） -->
      <el-table-column prop="endYear" label="结题年份" width="180" align="center">
        <template v-slot="scope">
          <el-input 
            v-if="scope.row.isEditing" 
            v-model.number="scope.row.endYear" 
            size="small" 
            type="number" 
            min="2000" 
            max="2100"
...
export default {
  ...
  methods: {
    ...
    addRow() {
      this.tableData.push({
        id: null,
        projectName: '', // 项目名称（空字符串初始化）
        endYear: new Date().getFullYear(), // 结题年份（默认当前年份）
        level: '国家级', // 级别（默认国家级）
        workload: 0, // 工作量（默认0）
        pdfUrl: '',
        isEditing: true
    ...
    async saveRow(row) {
      ...
      if (!row.endYear || row.endYear < 2000 || row.endYear > new Date().getFullYear() + 1) {
        return ElMessage.warning('请输入有效结题年份（2000-'+(new Date().getFullYear() + 1)+'）');
      }
      ...
      this.$emit('save-row', 'scienceInnovation', row); // 模块标识：scienceInnovation
    },
    ...
    deleteRow(rowId, index) {
      this.tableData.splice(index, 1);
      this.$emit('delete-row', 'scienceInnovation', rowId);
    },
