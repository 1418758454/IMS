<template>
  <el-card class="module-card">
    ...
    <div class="module-header">
      <h3 class="module-title">十二、研究生教学（指导学生）</h3>
      <!-- 新增按钮：只在非审核模式下显示 -->
      <el-button 
        v-if="mode !== 'check'" 
        type="primary" 
    ...
    >
      ...
      <!-- 学生数（数字输入） -->
      <el-table-column prop="studentCount" label="学生数" width="150" align="center">
        <template v-slot="scope">
          <el-input 
            ...
            class="edit-input"
          ></el-input>
          <span v-else>{{ scope.row.studentCount || 0 }}</span>
        </template>
      </el-table-column>

      <!-- 学生类型（下拉框+系数绑定） -->
      <el-table-column prop="studentType" label="学生类型" width="200" align="center">
        <template v-slot="scope">
          <el-select 
            v-if="scope.row.isEditing" 
            v-model="scope.row.studentType" 
            size="small" 
            placeholder="请选择"
            class="edit-select"
            @change="handleStudentTypeChange(scope.row)"
          >
            <el-option label="一年级学硕" value="一年级学硕"></el-option>
            <el-option label="二年级学硕" value="二年级学硕"></el-option>
            <el-option label="三年级学硕" value="三年级学硕"></el-option>
            <el-option label="一年级专硕" value="一年级专硕"></el-option>
            <el-option label="二年级专硕" value="二年级专硕"></el-option>
            <el-option label="三年级专硕" value="三年级专硕"></el-option>
            <el-option label="一年级博士" value="一年级博士"></el-option>
            <el-option label="二年级博士" value="二年级博士"></el-option>
            <el-option label="三年级博士" value="三年级博士"></el-option>
            <el-option label="四年级博士" value="四年级博士"></el-option>
          </el-select>
          <span v-else>
      ...
      <!-- 系数（自动绑定，不可编辑） -->
      <el-table-column prop="coefficient" label="系数" width="120" align="center">
        <template v-slot="scope">
          <span>{{ scope.row.coefficient || 0 }}</span>
        </template>
      </el-table-column>

      <!-- 工作量（手动输入） -->
      <el-table-column prop="workload" label="工作量（请手动输入）" min-width="180" align="center">
        <template v-slot="scope">
          <el-input 
            v-if="scope.row.isEditing" 
...
export default {
  ...
  methods: {
    ...
    addRow() {
      this.tableData.push({
        id: null,
        studentCount: 0,          // 学生数默认0
        studentType: '一年级学硕',   // 默认一年级学硕
        coefficient: 30,          // 默认系数30（与一年级学硕绑定）
        workload: 0,              // 工作量默认0
        isEditing: true
      });
    },
    ...
    editRow(row) {
      ...
      row.isEditing = true; 
      // 重新计算系数（确保编辑时系数与类型匹配）
      row.coefficient = this.studentTypeCoefficient[row.studentType] || 0;
    },

    // 学生类型变更时更新系数
    handleStudentTypeChange(row) {
      row.coefficient = this.studentTypeCoefficient[row.studentType] || 0;
    },
    ...
    saveRow(row) {
      // 校验必填项
      if (row.studentCount < 0) return ElMessage.warning('学生数不能为负数');
      if (!row.studentType) return ElMessage.warning('请选择学生类型');
      ...
      this.$emit('save-row', 'graduateGuideStudent', row); // 模块标识：graduateStudent
    },
    ...
    deleteRow(rowId, index) {
      this.tableData.splice(index, 1);
      this.$emit('delete-row', 'graduateGuideStudent', rowId);
    },
