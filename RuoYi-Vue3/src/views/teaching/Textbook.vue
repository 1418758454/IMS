<template>
  <el-card class="module-card">
    ...
    <div class="module-header">
      <h3 class="module-title">七、出版教材</h3>
      <!-- 新增按钮：只在非审核模式下显示 -->
      <el-button 
        v-if="mode !== 'check'" 
        type="primary" 
    ...
    >
      ...
      <!-- 教材名称 -->
      <el-table-column prop="textbookName" label="教材名称" width="220" align="center">
        <template v-slot="scope">
          <el-input 
            v-if="scope.row.isEditing" 
            v-model="scope.row.textbookName" 
            size="small" 
            placeholder="请输入教材名称"
            class="edit-input"
          ></el-input>
          <span v-else>{{ scope.row.textbookName || '-' }}</span>
        </template>
      </el-table-column>

      <!-- 出版社名称 -->
      <el-table-column prop="pressName" label="出版社名称" width="220" align="center">
        <template v-slot="scope">
          <el-input 
            v-if="scope.row.isEditing" 
            v-model="scope.row.pressName" 
            size="small" 
            placeholder="请输入出版社名称"
            class="edit-input"
          ></el-input>
          <span v-else>{{ scope.row.pressName || '-' }}</span>
        </template>
      </el-table-column>

      <!-- 出版时间 -->
      <el-table-column prop="publishDate" label="出版时间" width="180" align="center">
        <template v-slot="scope">
          <el-date-picker 
            v-if="scope.row.isEditing" 
            ...
            class="edit-input"
          ></el-date-picker>
          <span v-else>{{ scope.row.publishDate || '-' }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="level" label="级别" width="180" align="center">
        <template v-slot="scope">
          <el-select 
            ...
            class="edit-select"
          >
            <el-option label="国家级规划教材" value="国家级规划教材"></el-option>
            <el-option label="省部级规划教材" value="省部级规划教材"></el-option>
            <el-option label="一般教材" value="一般教材"></el-option>
          </el-select>
      ...
      <!-- 教材编写身份 -->
      <el-table-column prop="compilerIdentity" label="教材编写身份" width="180" align="center">
        <template v-slot="scope">
          <el-select 
            v-if="scope.row.isEditing" 
            v-model="scope.row.compilerIdentity" 
            size="small" 
            placeholder="请选择身份"
            class="edit-select"
          >
            ...
            <el-option label="参编" value="参编"></el-option>
          </el-select>
          <span v-else>{{ scope.row.compilerIdentity || '-' }}</span>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    addRow() {
      this.tableData.push({
        ...
        textbookName: '', // 教材名称
        pressName: '', // 出版社名称
        publishDate: new Date().toISOString().split('T')[0], // 出版时间（默认今天）
        level: '国家级规划教材', // 级别
        compilerIdentity: '主编', // 编写身份
        workload: 0, 
        pdfUrl: '',
        isEditing: true
      });
    ...
    async saveRow(row) {
      // 校验
      if (!row.textbookName.trim()) return ElMessage.warning('请输入教材名称');
      if (!row.pressName.trim()) return ElMessage.warning('请输入出版社名称');
      if (!row.publishDate) return ElMessage.warning('请选择出版时间');
      if (!row.level) return ElMessage.warning('请选择级别');
      if (!row.compilerIdentity) return ElMessage.warning('请选择教材编写身份');
      ...
      this.$emit('save-row', 'textbook', row); // 模块标识：textbook
    },
    ...
    deleteRow(rowId, index) {
      this.tableData.splice(index, 1);
      this.$emit('delete-row', 'textbook', rowId);
    },
