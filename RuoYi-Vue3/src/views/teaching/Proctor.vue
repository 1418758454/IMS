<template>
  <el-card class="module-card">
    ...
    <div class="module-header">
      <h3 class="module-title">十、监考模块</h3>
      <!-- 新增按钮：只在非审核模式下显示 -->
      <el-button 
        v-if="mode !== 'check'" 
        type="primary" 
    ...
    >
      ...
      <!-- 监考类型（下拉选择） -->
      <el-table-column prop="examType" label="监考类型" width="200" align="center">
        <template v-slot="scope">
          <el-select 
            ...
            class="edit-select"
          >
            <el-option label="课程考试" value="课程考试"></el-option>
            <el-option label="补考" value="补考"></el-option>
            <el-option label="其他考试" value="其他考试"></el-option>
          </el-select>
      ...
      <!-- 本年度监考次数（数字输入） -->
      <el-table-column prop="examCount" label="本年度监考次数" width="180" align="center">
        <template v-slot="scope">
          <el-input-number 
            v-if="scope.row.isEditing" 
            ...
            class="edit-input"
          ></el-input-number>
          <span v-else>{{ scope.row.examCount || 0 }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    addRow() {
      this.tableData.push({
        id: null,
        examType: '', // 监考类型（空字符串初始化）
        examCount: 0, // 监考次数（默认0）
        workload: 0, // 工作量（默认0）
        isEditing: true
      });
    },
    ...
    saveRow(row) {
      // 校验必填项
      if (!row.examType) return ElMessage.warning('请选择监考类型');
      if (row.examCount === null || row.examCount < 0) {
        return ElMessage.warning('请输入有效监考次数（≥0）');
      }
      ...
      this.$emit('save-row', 'proctor', row); // 模块标识：proctor
    },
    ...
    deleteRow(rowId, index) {
      this.tableData.splice(index, 1);
      this.$emit('delete-row', 'proctor', rowId);
    },
