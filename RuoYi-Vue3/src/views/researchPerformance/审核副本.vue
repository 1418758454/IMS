<template>
  <div class="research-performance">
    ...
    <el-card class="module-card annual-workload-card">
      ...
      <el-table :data="yearWorkloadTableData" border class="summary-table" >
        <el-table-column label="课题工作量" min-width="120" align="center">
          <template v-slot="scope">{{ scope.row.subjectWorkload.toFixed(3) }}</template>
        </el-table-column>
        <el-table-column label="论文工作量" min-width="120" align="center">
          <template v-slot="scope">{{ scope.row.paperWorkload.toFixed(3) }}</template>
        </el-table-column>
        <el-table-column label="论著工作量" min-width="120" align="center">
          <template v-slot="scope">{{ scope.row.monographWorkload.toFixed(3) }}</template>
        </el-table-column>
        <el-table-column label="获奖工作量" min-width="120" align="center">
          <template v-slot="scope">{{ scope.row.awardWorkload.toFixed(3) }}</template>
        </el-table-column>
        <el-table-column label="专利工作量" min-width="120" align="center">
          <template v-slot="scope">{{ scope.row.patentWorkload.toFixed(3) }}</template>
        </el-table-column>
        <el-table-column label="软著工作量" min-width="120" align="center">
          <template v-slot="scope">{{ scope.row.softwareWorkload.toFixed(3) }}</template>
        </el-table-column>
    ...
    <el-card class="module-card">
      ...
      <el-table :data="subjectTableData" border class="module-table" ref="subjectTableRef" v-loading="loading.subject">
        ...
        <el-table-column label="工号" prop="userId" align="center">
          <template v-slot="scope"><span>{{ scope.row.userId }}</span></template>
        </el-table-column>
      ...
      <div class="module-summary">
        ...
        <span class="summary-value">{{ subjectTotalWorkload.toFixed(3) }} 工作量</span>
      </div>
    </el-card>
    ...
    <el-card class="module-card">
      ...
      <div class="module-summary">
        <span class="summary-label">模块合计：</span>
        <span class="summary-value">{{ paperTotalWorkload.toFixed(3) }} 工作量</span>
      </div>
    </el-card>
    ...
    <el-card class="module-card">
      ...
      <div class="module-summary">
        <span class="summary-label">模块合计：</span>
        <span class="summary-value">{{ monographTotalWorkload.toFixed(3) }} 工作量</span>
      </div>
    </el-card>
    ...
    <el-card class="module-card">
      ...
      <div class="module-summary">
        <span class="summary-label">模块合计：</span>
        <span class="summary-value">{{ awardTotalWorkload.toFixed(3) }} 工作量</span>
      </div>
    </el-card>
    ...
    <el-card class="module-card">
      ...
      <div class="module-summary">
        <span class="summary-label">模块合计：</span>
        <span class="summary-value">{{ patentTotalWorkload.toFixed(3) }} 工作量</span>
      </div>
    </el-card>
    ...
    <el-card class="module-card">
      ...
      <div class="module-summary">
        <span class="summary-label">模块合计：</span>
        <span class="summary-value">{{ softwareTotalWorkload.toFixed(3) }} 工作量</span>
      </div>
    </el-card>
...
import { saveYearWorkload } from '@/api/researchPerformance/totalWorkload.js'; // 根据实际路径调整
import { parse } from '@vue/compiler-sfc';
import { getUserListByDept,auditSubject } from '@/api/check/researchCheck.js';
...
export default {
  ...
  methods: {
    ...
    handleReset() {
      ...
      this.filterForm.year = new Date().getFullYear().toString();
      // 重置为第一个部门
      if (this.deptList.length > 0) {
        this.filterForm.deptId = null;
        this.filterForm.userId = ''; // 重置为全部用户
    ...
    getSubjectData() {
      ...
      const params = {
        ...
        userId: this.filterForm.userId || undefined // 不传userId表示查询部门所有用户
      };
    ...
    initEditFormData(module, row) {
     ...
     submitModuleData(module, submitData) {
      if (!this.isEdit && this.filterForm.userId) {
        submitData.userId = this.filterForm.userId;
      }
      // 获取对应模块的API
      const apiMap = {
    ...
    calculateYearWorkload() {
      // 构造年度汇总数据（单个对象）
      const summaryData = {
        year: this.selectedYear || '所有年份',  // 当前选中年份
        subjectWorkload: this.subjectTotalWorkload, // 课题模块总和
        paperWorkload: this.paperTotalWorkload,     // 论文模块总和
        monographWorkload: this.monographTotalWorkload, // 论著模块总和
        awardWorkload: this.awardTotalWorkload,     // 获奖模块总和
        patentWorkload: this.patentTotalWorkload,   // 专利模块总和
        softwareWorkload: this.softwareTotalWorkload, // 软著模块总和
      };
      // // 计算总工作量（各模块之和）
    ...
    // 在 methods 中添加新方法
    updateYearWorkloadData() { //todo 避免保存到数据库
      // 刷新所有模块数据
