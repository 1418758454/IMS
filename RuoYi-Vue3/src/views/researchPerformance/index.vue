      ...
      <div class="module-summary">
        ...
        <div class="summary-content">
          ...
          <div class="summary-row">
            ...
            <span class="summary-value estimated">{{ subjectEstimatedWorkload.toFixed(3) }} 工作量</span>
          </div>
        </div>
      </div>
    </el-card>
    ...
    <el-card class="module-card">
      ...
      <div class="module-summary">
        ...
        <div class="summary-content">
          ...
          <div class="summary-row">
            <span class="summary-label">预计总工作量：</span>
            <span class="summary-value estimated">{{ paperEstimatedWorkload.toFixed(3) }} 工作量</span>
          </div>
        </div>
      </div>
    </el-card>
    ...
    <el-card class="module-card">
      ...
      <div class="module-summary">
        ...
        <div class="summary-content">
          ...
          <div class="summary-row">
            <span class="summary-label">预计总工作量：</span>
            <span class="summary-value estimated">{{ monographEstimatedWorkload.toFixed(3) }} 工作量</span>
          </div>
        </div>
      </div>
    </el-card>
    ...
    <el-card class="module-card">
      ...
      <div class="module-summary">
        ...
        <div class="summary-content">
          ...
          <div class="summary-row">
            <span class="summary-label">预计总工作量：</span>
            <span class="summary-value estimated">{{ awardEstimatedWorkload.toFixed(3) }} 工作量</span>
          </div>
        </div>
      </div>
    </el-card>
    ...
    <el-card class="module-card">
      ...
      <div class="module-summary">
        ...
        <div class="summary-content">
          ...
          <div class="summary-row">
            <span class="summary-label">预计总工作量：</span>
            <span class="summary-value estimated">{{ patentEstimatedWorkload.toFixed(3) }} 工作量</span>
          </div>
        </div>
      </div>
    </el-card>
    ...
    <el-card class="module-card">
      ...
      <div class="module-summary">
        ...
        <div class="summary-content">
          ...
          <div class="summary-row">
            <span class="summary-label">预计总工作量：</span>
            <span class="summary-value estimated">{{ softwareEstimatedWorkload.toFixed(3) }} 工作量</span>
          </div>
        </div>
      </div>
    </el-card>
    ...
    >
      ...
      >
        ...
        <template v-if="currentModule === 'subject'">
          ...
          <!-- PDF上传 -->
          <el-form-item label="立项合同（PDF/图片）" prop="pdfUrl" required>
            <el-upload
        ...
        <template v-if="currentModule === 'paper'">
          ...
          <!-- PDF上传 -->
          <el-form-item label="论文原件（PDF/图片）" prop="pdfUrl" required>
            <el-upload
              v-evidence-file-opening
              class="upload-pdf"
              ref="pdfUpload"
        ...
        <template v-if="currentModule === 'monograph'">
          ...
          <!-- PDF上传 -->
          <el-form-item label="封面、目录及封底扫描件（PDF/图片）" prop="pdfUrl" required>
            <el-upload
              v-evidence-file-opening
              class="upload-pdf"
              ref="pdfUpload"
        ...
        <template v-if="currentModule === 'award'">
          ...
          <!-- PDF上传 -->
          <el-form-item label="证书（PDF/图片）" prop="pdfUrl" required>
            <el-upload
              v-evidence-file-opening
              class="upload-pdf"
              ref="pdfUpload"
...
import { saveYearWorkload,getResearchTotalWorkload  } from '@/api/researchPerformance/totalWorkload.js'; // 根据实际路径调整
...
export default {
  ...
  data() {
    return {
      ...
      summaryLoading: false, // 新增：汇总数据加载状态
      // 课题模块
      subjectTableData: [],
      subjectEstimatedWorkload: 0, // 预计总工作量（所有数据）
      subjectConfirmedWorkload: 0, // 已确认总工作量（仅已审核通过的数据）
      // 论文模块
      paperTableData: [],
      paperEstimatedWorkload: 0,
      paperConfirmedWorkload: 0, 
      // 论著模块
      monographTableData: [],
      monographEstimatedWorkload: 0,
      monographConfirmedWorkload: 0,
      // 获奖模块
      awardTableData: [],
      awardEstimatedWorkload: 0,
      awardConfirmedWorkload: 0,
      // 专利模块
      patentTableData: [],
      patentEstimatedWorkload: 0,
      patentConfirmedWorkload: 0,
      // 软著模块
      softwareTableData: [],
      softwareEstimatedWorkload: 0,
      ...
      // 年度工作量汇总静态数据
      selectedYear: new Date().getFullYear() - 1, // 默认选中上一年份
      years: [],
  ...
  methods: {
    ...
    getSubjectData() {
      const params = {
        year: this.selectedYear, // 新增：传递年份参数
        isAudit: false,
      };
      ...
      return this.getModuleData(
        ...
        'subjectEstimatedWorkload', // 预计总工作量字段
    ...
    getMonographData() { 
      ...
      return this.getModuleData(
        ...
        'monographEstimatedWorkload', // 总工作量字段
    ...
    getPatentData() { 
      ...
      return this.getModuleData(
        ...
        'patentEstimatedWorkload', // 总工作量字段
    ...
    getSoftwareData() { 
      ...
      return this.getModuleData(
        ...
        'softwareEstimatedWorkload', // 总工作量字段
    ...
    async fetchTotalWorkload(year, deptId = undefined, userId = undefined) {
      this.summaryLoading = true;
      try {
        // 调用后端汇总接口（参数与statistics页面一致）
        const response = await getResearchTotalWorkload({ year, deptId, userId });
        // 赋值年度汇总表格数据
        this.yearWorkloadTableData = [response.data];
        // 映射后端字段到模块变量（保持原有变量名）
        this.subjectConfirmedWorkload = response.data.subjectConfirmedWorkload || 0;
        this.subjectEstimatedWorkload = response.data.subjectEstimatedWorkload || 0;
        this.paperConfirmedWorkload = response.data.paperConfirmedWorkload || 0;
        this.paperEstimatedWorkload = response.data.paperEstimatedWorkload || 0;
        this.monographConfirmedWorkload = response.data.monographConfirmedWorkload || 0;
        this.monographEstimatedWorkload = response.data.monographEstimatedWorkload || 0;
        this.awardConfirmedWorkload = response.data.awardConfirmedWorkload || 0;
        this.awardEstimatedWorkload = response.data.awardEstimatedWorkload || 0;
        this.patentConfirmedWorkload = response.data.patentConfirmedWorkload || 0;
        this.patentEstimatedWorkload = response.data.patentEstimatedWorkload || 0;
        this.softwareConfirmedWorkload = response.data.softwareConfirmedWorkload || 0;
        this.softwareEstimatedWorkload = response.data.softwareEstimatedWorkload || 0;
      } catch (error) {
        this.$message.error(`汇总数据加载失败：${error.message || '请联系管理员'}`);
        this.yearWorkloadTableData = [];
        // 重置模块工作量
        Object.keys(this).forEach(key => {
          if (key.includes('ConfirmedWorkload') || key.includes('EstimatedWorkload')) {
            this[key] = 0;
          }
        });
      } finally {
     ...
     * @param {String} totalWorkloadKey - 总工作量存储字段（如 'paperEstimatedWorkload'）
    ...
    getModuleData(module, apiFunc, tableKey, totalKey, confirmedKey, errorMsg, extraParams = {}) {
      ...
      return apiFunc(params) // 接口调用时传递年份
        .then(response => {
          ...
          // // 使用后端返回的工作量数据
          // const estimatedKey = `${module}EstimatedWorkload`; // 如 subjectEstimatedWorkload
          // const confirmedKeyBack = `${module}ConfirmedWorkload`; // 如 subjectConfirmedWorkload

          // // 从响应数据中获取预计总工作量和已确认总工作量
          // this[totalKey] = response.data[estimatedKey] || 0;
          // this[confirmedKey] = response.data[confirmedKeyBack] || 0;
          ...
          // // 计算已确认总工作量（仅已通过状态）
          // this[confirmedKey] = response.data.records
     ...
     */
    // getYearWorkloadSummary(year) {
    //   return getYearWorkloadSummary({ year: year })
    //     .then(response => {
    //       if (response.code === 200) {
    //         const summaryData = response.data;
            
    //         // 绑定各模块合计工作量数据
    //         this.bindModuleWorkloadData(summaryData);
            
    //         // 绑定年度汇总表格数据
    //         this.bindYearSummaryData(summaryData);
            
    //         return summaryData;
    //       } else {
    //         this.$message.error(response.msg || '获取年度工作量汇总数据失败');
    //         return null;
    //       }
    //     })
    //     .catch(error => {
    //       this.$message.error('获取年度工作量汇总数据失败');
    //       console.error('获取年度工作量汇总数据失败:', error);
    //       // 设置默认值
    //       this.setDefaultWorkloadData();
    //       return null;
    //     });
    // },
    /**
     * 获取年度汇总数据（与统计页面共用同一接口）
     * @param {String} year - 年份
     * @param {Number} deptId - 部门ID（可选，默认undefined）
     * @param {Number} userId - 用户ID（可选，默认undefined）
     */
    async getYearWorkloadSummary(year, deptId = undefined, userId = undefined) {
      this.summaryLoading = true;
      try {
        // 调用统计页面相同的汇总接口
        const response = await getResearchTotalWorkload({ year, deptId, userId });
        if (response.code === 200) {
          ...
          this.$message.error(response.msg || '获取汇总数据失败');
          return null;
        }
      } catch (error) {
        this.$message.error('获取汇总数据失败');
        console.error('汇总数据接口异常:', error);
        this.setDefaultWorkloadData(); // 异常时设置默认值
        return null;
      } finally {
        this.summaryLoading = false;
      }
     ...
     */
    bindModuleWorkloadData(summaryData) {
      // 课题模块
      this.subjectEstimatedWorkload = summaryData.subjectEstimatedWorkload || 0;
      this.subjectConfirmedWorkload = summaryData.subjectConfirmedWorkload || 0;
      
      // 论文模块
      this.paperEstimatedWorkload = summaryData.paperEstimatedWorkload || 0;
      this.paperConfirmedWorkload = summaryData.paperConfirmedWorkload || 0;
      
      // 论著模块
      this.monographEstimatedWorkload = summaryData.monographEstimatedWorkload || 0;
      this.monographConfirmedWorkload = summaryData.monographConfirmedWorkload || 0;
      
      // 获奖模块
      this.awardEstimatedWorkload = summaryData.awardEstimatedWorkload || 0;
      this.awardConfirmedWorkload = summaryData.awardConfirmedWorkload || 0;
      
      // 专利模块
      this.patentEstimatedWorkload = summaryData.patentEstimatedWorkload || 0;
      this.patentConfirmedWorkload = summaryData.patentConfirmedWorkload || 0;
      
      // 软著模块
      this.softwareEstimatedWorkload = summaryData.softwareEstimatedWorkload || 0;
      this.softwareConfirmedWorkload = summaryData.softwareConfirmedWorkload || 0;
    },
     ...
     * @param {Object} summaryData - 汇总数据
     */
    bindYearSummaryData(summaryData) {
      this.yearWorkloadTableData = [{
        year: summaryData.year || this.selectedYear,
        subjectConfirmedWorkload: summaryData.subjectConfirmedWorkload || 0,
        paperConfirmedWorkload: summaryData.paperConfirmedWorkload || 0,
        monographConfirmedWorkload: summaryData.monographConfirmedWorkload || 0,
        awardConfirmedWorkload: summaryData.awardConfirmedWorkload || 0,
        patentConfirmedWorkload: summaryData.patentConfirmedWorkload || 0,
        softwareConfirmedWorkload: summaryData.softwareConfirmedWorkload || 0,
         totalWorkload: this.calculateTotalConfirmedWorkload(summaryData)
      }];
    },
     ...
     * @param {Object} summaryData - 汇总数据
     */
    calculateTotalConfirmedWorkload(summaryData) {
      return (summaryData.subjectConfirmedWorkload || 0) +
            (summaryData.paperConfirmedWorkload || 0) +
            (summaryData.monographConfirmedWorkload || 0) +
            (summaryData.awardConfirmedWorkload || 0) +
            (summaryData.patentConfirmedWorkload || 0) +
            (summaryData.softwareConfirmedWorkload || 0);
    },

    /**
     * 设置默认工作量数据
     */
    setDefaultWorkloadData() {
      // 重置各模块工作量
      this.subjectEstimatedWorkload = 0;
      this.subjectConfirmedWorkload = 0;
      this.paperEstimatedWorkload = 0;
      this.paperConfirmedWorkload = 0;
      this.monographEstimatedWorkload = 0;
      this.monographConfirmedWorkload = 0;
      this.awardEstimatedWorkload = 0;
      this.awardConfirmedWorkload = 0;
      this.patentEstimatedWorkload = 0;
      this.patentConfirmedWorkload = 0;
      this.softwareEstimatedWorkload = 0;
      this.softwareConfirmedWorkload = 0;
      
      // 重置年度汇总表格
      this.yearWorkloadTableData = [{
        year: this.selectedYear,
        subjectConfirmedWorkload: 0,
        paperConfirmedWorkload: 0,
        monographConfirmedWorkload: 0,
        awardConfirmedWorkload: 0,
        patentConfirmedWorkload: 0,
    ...
    // },
     submitModuleData(module, submitData) {
      ...
      api(submitData)
        .then(res => {
          if (res.code === 200) {
            const successMessage = res.msg || (this.isEdit ? '更新成功' : '新增成功');
            this.dialogVisible = false;
            this.refreshModuleData(module);
            // 刷新汇总数据
            this.getYearWorkloadSummary(this.selectedYear);
            // this.updateYearWorkloadData();
            
            // 重置编辑状态
            this.isEdit = false;
            this.editRowId = null;
            this.showSaveResult(successMessage);
          } else {
    ...
    /**
     * 年份下拉框变化事件
     * @param {string} year - 选中的年份（2024/2025/2026）
     */
    handleYearChange(year) {
      this.selectedYear = year;
      this.refreshAllModules(); // 刷新所有模块数据
      // this.calculateYearWorkload(); // 重新计算年度汇总
      // this.updateYearWorkloadData();
    },
    ...
    refreshAllModules() {
      return new Promise((resolve) => {
        Promise.all([
          ...
          this.fetchTotalWorkload(this.selectedYear);
          resolve();
        });
      });
    },
    ...
    //       // 预计工作量（所有数据）
    //       subjectEstimatedWorkload: this.subjectEstimatedWorkload,
    //       paperEstimatedWorkload: this.paperEstimatedWorkload,
    //       monographEstimatedWorkload: this.monographEstimatedWorkload,
    //       awardEstimatedWorkload: this.awardEstimatedWorkload,
    //       patentEstimatedWorkload: this.patentEstimatedWorkload,
    //       softwareEstimatedWorkload: this.softwareEstimatedWorkload,
    //       totalEstimatedWorkload: this.subjectEstimatedWorkload + 
    //                     this.paperEstimatedWorkload + 
    //                     this.monographEstimatedWorkload + 
    //                     this.awardEstimatedWorkload + 
    //                     this.patentEstimatedWorkload + 
    //                     this.softwareEstimatedWorkload,
          
    //       // 已确认工作量（仅已通过状态）
    //       subjectConfirmedWorkload: this.subjectConfirmedWorkload,
    //       paperConfirmedWorkload: this.paperConfirmedWorkload,
    //       monographConfirmedWorkload: this.monographConfirmedWorkload,
    //       awardConfirmedWorkload: this.awardConfirmedWorkload,
    //       patentConfirmedWorkload: this.patentConfirmedWorkload,
    //       softwareConfirmedWorkload: this.softwareConfirmedWorkload,
    //       totalConfirmedWorkload: this.subjectConfirmedWorkload + 
    //                             this.paperConfirmedWorkload + 
    //                             this.monographConfirmedWorkload + 
    //                             this.awardConfirmedWorkload + 
    //                             this.patentConfirmedWorkload + 
    //                             this.softwareConfirmedWorkload
    //     };
