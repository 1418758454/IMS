...
/**
 * 研究生指导学生Mapper接口（复用BaseMapper通用方法）
 */
@Mapper
public interface GraduateGuideStudentMapper extends BaseMapper<GraduateGuideStudent> {

    // 查询数据库中所有id为userId的记录，将其用户名改为userName
    @Update("update teaching_graduate_guide_student set user_name = #{userName} where user_id = #{userId}")
