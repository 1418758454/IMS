...
public interface BasicInformationMapper extends BaseMapper<BasicInformation> {
    ...
    @Insert("insert into user_registration (user_Id, name, gender, birth_date, native_place, political_status,title, current_position, undergrad_school, undergrad_time, highest_education, highest_education_time, overseas_experience, research_direction, phone, email, wechat, qq, personal_website, password, status, department, position) values (#{userId}, #{name}, #{gender}, #{birthDate}, #{nativePlace}, #{politicalStatus}, #{title}, #{currentPosition}, #{undergradSchool}, #{undergradTime}, #{highestEducation}, #{highestEducationTime}, #{overseasExperience}, #{researchDirection}, #{phone}, #{email}, #{wechat}, #{qq}, #{personalWebsite}, #{password}, #{status}, #{department}, #{position})")
